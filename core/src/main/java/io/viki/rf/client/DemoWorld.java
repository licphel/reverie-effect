...
import io.viki.rf.network.world.ClientWorldSync;
...
final class DemoWorld implements AutoCloseable {
  ...
  ClientWorldSync worldSync() {
    ClientSession current = client;
    if (current == null) {
      throw new IllegalStateException("Client session has not started");
    }
    return current.worldSync();
  }
